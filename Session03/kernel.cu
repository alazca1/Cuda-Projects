...
__global__ void threshold_kernel_2d(byte* img_gray_in, byte* img_gray_out, int w, int h, byte th1, byte th2)
{
	...
	int y = threadIdx.y + 1 + (blockIdx.y * blockDim.y);
...
void ejercicio_03_1_01_GPU_th_erode()
{
	...
	int THREADS_PER_BLOCK_X = mmTHREADS_PER_BLOCK_X + 2;
	int THREADS_PER_BLOCK_Y = mmTHREADS_PER_BLOCK_Y + 2;
	dim3 dim_grid(NUM_BLOCKS_X, NUM_BLOCKS_Y);
	dim3 dim_block(THREADS_PER_BLOCK_X, THREADS_PER_BLOCK_Y);
	...
	// Allocate GPU memory
	cudaMalloc((void**)&dev_gray_01, rgb__img_sz * sizeof(byte));
	cudaMalloc((void**)&dev_gray_02, rgb__img_sz * sizeof(byte));
	cudaMalloc((void**)&dev_gray_03, rgb__img_sz * sizeof(byte));
	cudaMalloc((void**)&dev_rgb02, rgb__img_sz * sizeof(byte));
	cudaMalloc((void**)&dev_rgb03, rgb__img_sz * sizeof(byte));


	// Copy input data to GPU memory
	cudaMemcpy(dev_gray_01, grayscale_in, rgb__img_sz * sizeof(byte), cudaMemcpyHostToDevice);
	...
	// Write bmp file
	write_bmp(filename01, w, h, (char*)dev_gray_02);
